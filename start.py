...
class Download():
    def __init__( self, torrent_file_name, max_peers, download_path ):
        ...
        print("Number Of pieces", self.number_of_pieces)
        self.bitfield = []
        ...
        print(f"\nDownloading file {self.file_name} at location \"{self.download_path}\"\n")        
        ...
        self.all_peers = []
        self.active_peers = []
        print("\nAvailable peers: ", len(self.peer_list))
        myTable = PrettyTable(["IP", "Port"])
        for ip, port in self.peer_list:
            myTable.add_row([ip, port])
            p = Peer( ip, port, self.torrent.info_hash, self.torrent.peer_id )
            self.all_peers.append(p)
        print(myTable)
        print("\nNumber of seeders:",self.tracker.peer_data['seeders'])
        print("Number of leechers:\n",self.tracker.peer_data['leechers'])
